...
fn add_invoice_datas(po: &ProductOrder, cl: &PdfLayerReference, font: &IndirectFontRef, fb: &IndirectFontRef) {
    let mut today = Utc::today().to_string();
    today.truncate(today.len() - 3);
    ...
    cl.add_shape(square);
    cl.use_text("Facture n°:", 10.0, Mm(132.0), Mm(280.0), &fb);
    cl.use_text(&po.uuid, 10.0, Mm(135.0), Mm(275.0), &font);

    cl.use_text("Date de facture: ", 10.0, Mm(132.0), Mm(270.0), &fb);
    cl.use_text(today, 10.0, Mm(160.0), Mm(270.0), &font);
}
...
fn add_total_carter(cl: &PdfLayerReference, fb: &IndirectFontRef) {
    let square = Line { 
        points: vec![
            ...
            (Point::new(Mm(150.0), Mm(30.0)), false)
        ],
        is_closed: true, 
        has_fill: false,
        has_stroke: true,
    ...
    let line_5 = Line { 
        points: vec![
            ...
            (Point::new(Mm(175.0), Mm(30.0)), false),
    ...
    cl.use_text("Frais de port", 9.0, Mm(151.0), Mm(46.0), &fb);
    cl.use_text("Bon de réduction", 9.0, Mm(151.0), Mm(41.0), &fb);
    cl.use_text("TVA", 9.0, Mm(151.0), Mm(36.0), &fb);
    cl.use_text("20%", 9.0, Mm(177.0), Mm(36.0), &fb);
    cl.use_text("Total TTC", 9.0, Mm(151.0), Mm(31.0), &fb);

    cl.use_text("Payé par carte bleu le 02/01/1991", 9.0, Mm(10.0), Mm(50.0), &fb);
}

fn add_company_informations(cl: &PdfLayerReference, fb: &IndirectFontRef) {
    let informations_line_1_0 = "SAS société du pneu - 19 boulevard des pneumatiques Gironde";
    let informations_line_1_1 = " - CAPITAL DE 1000.00€ - Carcassone - SIRET: 000 000 000 00000";
    let informations_line_1_2 = " - TVA N° FR00 000 000 000 - APE: 4511 Z";
    let informations_line_2 = "Facture soumise aux Conditions Générales de vente de la société du pneumatique";
...
fn display_products(po: ProductOrder, doc: &PdfDocumentReference, mut cl: PdfLayerReference, font: &IndirectFontRef, fi: &IndirectFontRef) {
    ...
    main_cl.use_text(format!("{:.2}€", po.total + po.total * 0.2), 9.0, Mm(177.0), Mm(31.0), &font);
}
